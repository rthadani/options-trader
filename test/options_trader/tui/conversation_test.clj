...
(clojure.test/use-fixtures :each isolate-state)
...
(deftest record-claude-session-is-idempotent-on-same-id
  ...
  (conv/record-claude-session! "AAPL" "sess-1")
  (is (= "sess-1" (:claude-session-id (conv/current-claude-session "AAPL")))))
...
(deftest persist-and-load-round-trips
  (let [path (str (System/getProperty "java.io.tmpdir")
                  "/options-trader-test-" (System/currentTimeMillis) ".edn")]
    (try
      (conv/record-claude-session! "AAPL" "sess-aapl")
...
(defn- write-jsonl [^File f lines]
  (.mkdirs (.getParentFile f))
  (spit f (str (str/join "\n" lines) "\n")))

(deftest replay-turns-claude-keeps-text-skips-noise
  (let [f (File. (System/getProperty "java.io.tmpdir")
    ...
    (try
      ...
      (let [turns (conv/replay-turns :claude (.getAbsolutePath f))]
        (is (= 3 (count turns)) "tool_result-only user turn and non-text blocks dropped")
        (is (= [:user :assistant :assistant] (mapv :role turns)))
        (is (= ["first question" "first answer" "second answer"] (mapv :text turns))))
      (finally (.delete f)))))
  ...
  (let [f (File. (System/getProperty "java.io.tmpdir")
    ...
    (try
      ...
      (let [turns (conv/replay-turns :pi (.getAbsolutePath f))]
        (is (= 2 (count turns)) "session/model-change events and toolResult dropped")
...
(deftest list-all-transcripts-discovers-both-agents-and-derives-session-ids
  (let [root  (File. (System/getProperty "java.io.tmpdir")
                     (str "ot-sessions-" (System/currentTimeMillis)))
        c-dir (File. root "claude/projects/-some-project")
        p-dir (File. root "pi/sessions")
        c-id  "11111111-1111-1111-1111-111111111111"
        p-id  "22222222-2222-2222-2222-222222222222"]
    (try
      (write-jsonl (File. c-dir (str c-id ".jsonl"))
                   ["{\"type\":\"system\",\"meta\":{}}"
                    "{\"type\":\"user\",\"message\":{\"role\":\"user\",\"content\":\"hello claude\"}}"])
      (write-jsonl (File. p-dir (str "2026-06-07T10-00-00Z_" p-id ".jsonl"))
                   ["{\"type\":\"message\",\"message\":{\"role\":\"user\",\"content\":[{\"type\":\"text\",\"text\":\"hello pi\"}]}}"])
      ;; Anything inside subagents/ is auxiliary and must NOT appear in the listing.
      (write-jsonl (File. c-dir "subagents/agent-xyz.jsonl")
                   ["{\"type\":\"user\",\"message\":{\"content\":\"nested helper\"}}"])
      (conv/record-claude-session! "AAPL" c-id)
      (let [rows (conv/list-all-transcripts
                   {:runtime-claude-dir (.getAbsolutePath (File. root "claude"))
